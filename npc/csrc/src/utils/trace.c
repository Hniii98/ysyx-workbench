...
/* Instruction ring buffer */
    ...
    void iringbuf_display(){
        ...
        printf("---- instructions ring buffer (capacity: %d defined by RBUFSIZE in trace.h)----\n", RBUFSIZE);
...
bool mtrace_enable(uint32_t addr){
    return MUXDEF(CONFIG_MTRACE, (addr >= 0x00000000) && 
        (addr <= 0xFFFFFFFF), false);
}

void mwrite_trace(uint32_t addr, int len, uint32_t data){
    ...
    printf("\tnemu: write lower %d bytes of { 0x%08x } to memory at " FMT_PADDR "\n",
        len,
        data,
        addr); 
}

void mread_trace(uint32_t addr, int len){
